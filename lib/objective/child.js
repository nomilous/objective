...
module.exports.run = function(root, config, objectiveFn, deferral) {
  ...
  return pipeline.emit('objective.starting', {
    ...
    try {
      ...
      var startIn = dirname(root.home + sep + config.filename);
      running = objective.injector({startIn: startIn}, objectiveFn);
      ...
      if ((running != null) && (running.then != null) && typeof running.then === 'function') {
        ...
        delete objective.childPromise;

      } else if (objective.childPromise) {

        handlePromise(objective.childPromise);
        delete objective.childPromise;
