...
module.exports = function(root, opts, fn) {
  ...
  var proxyPromise = promise(function(resolve, reject, notify) {
    ...
    inject = function(err) {
      ...
      // TODO, can still get injection error into runWriteArgs
      //       override opts.error?
      var promised
      try {
    ...
    sequence(args.map(function(arg){
          ...
          pipeline.emit('objective.injecting', {
            ...
            try {
              ...
              if (opts.moduleInjectFrom) {
                ...
                module.paths = build(opts.moduleInjectFrom,[]);
      ...
      function(err) {
        ...
        if (!opts.ignoreInjectError) return resolve();
        inject(err);
      }
    );
  });
